...
- **Monitoramento (blocos da materialização em `MATERIALIZE_FARM.sql` + scripts complementares):**
  - **Bloco 1 (produtos):** ao materializar as categorias de produto, o **bloco 1b** (`MONITOR_CARENCIA.sql`) garante uma **carência default por categoria** (`FARM_PRODUCT_CARENCIA_DEFAULT`, seed conservador: reentrada=1 dia p/ defensivos, 0 p/ não-químicos; colheita=0 p/ preencher pelo bulário) — só para categorias ainda sem default.
  - **Bloco 3 (pragas):** além do dedupe por nome (COLLATE), agora **deriva `FARM_PEST.category` do `target_type` do raw**: `plague→praga`, `disease→doenca`, `weed→erva_daninha` (categoria só é preenchida quando está NULL; `inimigo_natural` não existe no raw e vem do seed do CSV — bloco 4c). Também mantém a ponte `farmbox_target_id` (via `MONITOR_STOP_RESULTS.sql`).
  - **Bloco 4b (achados por parada):** `MONITOR_STOP_RESULTS.sql` materializa `FARM_MONITORING_STOP_RESULT` (rebuild) do raw `FARMBOX_MONITORING.record → monitoring_stops[].monitoring_stop_results[]` (lat/lon + praga + `quantity`), ~842k linhas — **fonte do mapa de calor**.
  - **Bloco 4c (limites de controle por cultura):** `MONITOR_THRESHOLDS_CSV.sql` faz o **seed dos limites POR CULTURA** a partir do CSV do Farmbox (`estagios_ativos`), com **Controle/Dano Vegetativo e Reprodutivo** (`action/damage_level` + `rep_action/rep_damage_level`) por (cultura, alvo, estágio). É **aditivo**: rebuild só do que tem `source='farmbox'`, **preserva** e **não ressuscita** o que o agrônomo editou/apagou (`source='app'`, tombstone). Cria os alvos faltantes (com categoria) e classifica `inimigo_natural`/`Pousio`.
  - **Bloco 4d (notas georreferenciadas):** `MONITOR_NOTES.sql` materializa `FARM_MONITORING_NOTE` (rebuild) de `FARMBOX_MONITORING_DAY_RESULT.record → $.notes[]` — lat/lon, descrição, `user_name` e `image_urls` (URLs S3) + `image_count`; ~57k notas (47k com foto). O talhão/plantio só é atribuído quando a nota é do tipo `Fields::Plantation` (senão fica só pelo lat/lon).
  - **GOTCHA (encoding):** o seed do CSV (`MONITOR_THRESHOLDS_CSV.sql`) **exige `sqlcmd -f 65001 -I`** (lê o arquivo como **UTF-8** + quoted identifiers). Sem `-f 65001` os nomes acentuados de cultura/alvo **corrompem** e não casam por nome (perde-se ~metade das culturas + cria alvos duplicados).
