...
### Módulo Agronômico nativo (FARM_*, 26 tabelas) — sucede o Farmbox
...
- **Catálogo de alvos:** `FARM_PEST` — pragas/doenças/daninhas/inimigos naturais (base de bulário e monitoramento); `category` (`doenca` | `erva_daninha` | `inimigo_natural` | `praga`) e `farmbox_target_id` (ponte para o `target.id` do Farmbox). `FARM_PEST_THRESHOLD` — **limites de controle por praga×parâmetro×cultura** (ver "Monitoramentos — limites de controle" abaixo).
...
- **Carência de insumos** (dois prazos do Farmbox — **Reentrada** = pessoa entrar no talhão; **Colheita**): `FARM_PRODUCT_CARENCIA_DEFAULT` — **default por categoria** de produto (`category_id`→`FARM_PRODUCT_CATEGORY`, `reentrada_days`, `colheita_days`; único por categoria). `FARM_PRODUCT_CARENCIA` — **override por produto/bulário** (`product_id`→`FARM_PRODUCT`, mesmos prazos; único por produto). Resolução (na `VW_MONITOR_FIELD_STATUS`): override(produto) › default(categoria); o **bloqueio de monitoramento usa a REENTRADA**. DDL em `SQL/MONITOR_CARENCIA.sql`.
...
- **Monitoramento:** `FARM_MONITORING` — monitoramento por talhão/plantio (metodologia/estágio/amostrador; ponte `farmbox_monitoring_id`). `FARM_MONITORING_POINT` — pontos/paradas com coordenada. `FARM_MONITORING_FINDING` — o índice medido (infestação por alvo). `FARM_MONITORING_DAY_MONITOR` — amostrador (quem monitorou) por `plantation_id`+dia (elo com a contagem).
  - `FARM_MONITORING_STOP_RESULT` — **achado por parada (ponto)**, fonte do **mapa de calor**: `monitoring_id`→`FARM_MONITORING`, lat/lon, `stop_date`, `pest_id`→`FARM_PEST`, `target_name`, `param_name`, `quantity` (+ `farmbox_stop_id`). Materializado do raw (`FARMBOX_MONITORING.record` → `monitoring_stops[].monitoring_stop_results[]`) — **~842k linhas**. Índices por `monitoring_id` e `pest_id`. DDL em `SQL/MONITOR_STOP_RESULTS.sql`.
  - `FARM_MONITORING_NOTE` — **notas georreferenciadas com fotos** (clique-no-ponto / camada de fotos do mapa): `field_id`/`planting_id` (só quando a nota é do tipo plantation), `note_date`, lat/lon, `description`, `user_name`, `image_urls` (JSON array de URLs S3) e `image_count` (+ pontes `farmbox_note_id`/`farmbox_plantation_id`). Materializado de `FARMBOX_MONITORING_DAY_RESULT.record → $.notes[]` — **~57k linhas (47k com foto)**. DDL em `SQL/MONITOR_NOTES.sql`.
...
Configuração que o agrônomo define e o **nosso** app de monitoramento obedece (DDL em `SQL/MODULE_MONITOR_V1.sql`; roda depois do MODULE_AGRO). Os EVENTOS reusam `FARM_MONITORING*` (acima); os **achados por ponto**, **notas com foto**, os **limites de controle** e a **carência de insumos** vêm dos scripts complementares `SQL/MONITOR_STOP_RESULTS.sql`, `SQL/MONITOR_NOTES.sql`, `SQL/MONITOR_THRESHOLDS_CSV.sql` e `SQL/MONITOR_CARENCIA.sql` (descritos acima, no Módulo Agronômico).
- `MONITOR_TOLERANCE_DEFAULT` — **default GLOBAL da tolerância por cultura** (id, `culture_id` único, `days`). O sistema já vem pré-configurado (Soja 7, Algodão 7, Milho 7, Braquiária 8…) — **sem ligação com fazenda nem variedade**. É a base: se a fazenda não cadastrou nada específico, cai aqui. Seed = moda dos dias por cultura no Farmbox.
- `MONITOR_TOLERANCE` — **exceções** que sobrepõem o default global, por escopo `farm_id`(NULL=todas) + `culture_id` + `variety_id`(NULL="cultura em geral"). Resolução (mais específico vence, senão cai no DEFAULT): variedade+fazenda › variedade › cultura+fazenda › **[DEFAULT global]**. Índice único filtrado por escopo. **Seed** = só as variedades cujo valor no Farmbox difere do default (as demais herdam o global) → hoje 2 linhas. Pontes `farmbox_culture_id`/`farmbox_variety_id`, farm 2112.
...
Para colunas, tipos, PKs, FKs, índices e checks exatos, consultar os scripts de DDL desta pasta: `SQL/SETUP_FULL.sql` (base) + `SQL/MODULE_AGRO_V1.sql` (agronômico `FARM_*`) + `SQL/MODULE_MONITOR_V1.sql` + `SQL/MONITOR_STOP_RESULTS.sql` + `SQL/MONITOR_NOTES.sql` + `SQL/MONITOR_CARENCIA.sql` + `SQL/MONITOR_THRESHOLDS_CSV.sql` (monitoramento) + `SQL/FLIGHT_LOG.sql` (aplicação aérea) + `SQL/FERT_EXPORT_PROFILES.sql` (perfis de exportação).
