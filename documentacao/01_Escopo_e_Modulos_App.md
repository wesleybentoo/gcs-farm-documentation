...
- **Cultivos › Estágios Fenológicos** (`admin/estagios-fenologicos`) — IMPLEMENTADA (read-only). **Catálogo de estágios fenológicos por cultura** (`FARM_PHENOLOGICAL_STAGE`, materializado do Farmbox): código/nome, **ordem** (position), classificação **Vegetativo/Reprodutivo**, descrição agronômica e a marca **ignora-infestação**. É a **fundação da fenologia** — o monitoramento (`FARM_MONITORING.phenological_stage_id`) já é vinculado a este catálogo (95,7% dos monitoramentos: 12.606/13.167; 216 estágios em 10 culturas). Filtro por cultura + busca. `GET /phenology/stages`. **Evoluções futuras** (não feitas): série temporal/curva (materializar `FARMBOX_PHENOLOGICAL_STAGE_SAMPLE`), mapa de estágio atual por talhão, fenologia acionável (limites/estimativa) e calendário esperado × realizado.
