...
- **Clima por Safra** (`meteorologia/clima-safra`) — IMPLEMENTADA. Consolidado de clima **por plantio** (safra·ciclo·variedade·talhão) para **exportação** (`WEATHER_SEASON_SUMMARY`, rollup). Tabela com precipitação, temperaturas (méd/mín/máx), umidade média, radiação, nº de dias com chuva, irrigação e a janela do plantio; **precip/temp/umidade/radiação calculadas do grid horário por talhão** (`FIELD_WEATHER_HOURLY`) na janela do plantio via **Recalcular do grid**; **irrigação é manual**; editar qualquer célula marca a linha como `manual` (não é mais sobrescrita pelo recálculo). Export **CSV**. Respeita fazenda+safra do cabeçalho. Endpoints: `GET /weather/season-climate`, `POST /weather/season-climate/recompute`, `PUT /weather/season-climate/:plantingId`. **Ressalva:** o grid só tem telemetria a partir de ~jun/2026 → safras antigas ficam manuais/vazias.
...
- **Cultivos › Estágios Fenológicos** (`admin/estagios-fenologicos`) — IMPLEMENTADA (editor). **Catálogo + GUIA DE CAMPO por estágio** (`FARM_PHENOLOGICAL_STAGE`, materializado do Farmbox): código/nome, ordem, classificação **Vegetativo/Reprodutivo**, descrição, **ignora-infestação**. O monitoramento (`FARM_MONITORING.phenological_stage_id`) já é vinculado (95,7% — 12.606/13.167; 216 estágios em 10 culturas). **Guia de campo curado pelo agrônomo** (o ETL do Farmbox NÃO sobrescreve): **dicas de identificação** (como reconhecer na planta), **faixa de dias após emergência**, **"não confundir com"** (estágios irmãos) e **galeria de mídia** (fotos/vídeos — upload pro nosso storage `FARM_MEDIA`, servido por `/media/:id` com cache imutável, OU URL externa). Objetivo: **munir o monitor na lavoura, offline** — a API já devolve `updated_at`+URLs estáveis pro futuro app de campo sincronizar/cachear. Endpoints: `GET /phenology/stages`, `PUT /phenology/stages/:id` (guia), `POST /phenology/stages/:id/media` (upload/URL), `DELETE /phenology/media/:id`, `GET /media/:id`. **Evoluções futuras** (não feitas): série temporal/curva (`FARMBOX_PHENOLOGICAL_STAGE_SAMPLE`), mapa de estágio atual por talhão, fenologia acionável (limites/estimativa), calendário esperado × realizado, e o **app de campo offline** em si.
...
> ⏸️ **PAUSADO (decisão do usuário 2026-07-08): não seguiremos com este módulo por enquanto.** O módulo está **construído, no ar e funcional** (back+front+DB+backfill histórico 49 ensaios/118 faixas — commits back `7f76316`, front `572c264`, docs `9e79a69`); apenas **não recebe evolução** até nova decisão. Nada foi removido: tabelas, endpoints `/research` e as telas continuam existindo (o grupo **Pesquisa** segue no menu). Para retomar, é só voltar a evoluir a partir daqui.
